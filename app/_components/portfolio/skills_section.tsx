    ...
    <section id="habilidades" className="rounded-3xl">
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 sm:gap-4 lg:gap-6">
        {copy.skills.map((skill) => (
          <article
            key={skill.title}
            className="relative flex min-h-24 flex-col justify-between overflow-hidden rounded-3xl border border-white/70 bg-[linear-gradient(160deg,rgba(255,255,255,0.95)_0%,rgba(239,246,255,0.88)_100%)] p-3 shadow-[0_18px_40px_-30px_rgba(37,99,235,0.32)] backdrop-blur-sm dark:border-slate-700 dark:bg-none dark:bg-slate-900 sm:min-h-36 sm:p-5 lg:min-h-48 lg:p-6"
          >
            ...
            <p className="mt-3 hidden text-sm leading-6 text-slate-600 dark:text-slate-200 sm:block">
              {skill.detail}
            </p>
            <p className="mt-3 text-xs leading-5 text-slate-600 dark:text-slate-200 sm:hidden">
              {skill.detail}
            </p>
          </article>
        ))}
